...
wkv_cuda = load(name="wkv", sources=["cuda/wkv_op_delta_t.cpp", "cuda/wkv_cuda_delta_t.cu"],
                verbose=True, extra_cuda_cflags=['-res-usage', '--maxrregcount 60', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}'])

class WKV(torch.autograd.Function):
    @staticmethod
    def forward(ctx, B, T, C, w, u, k, v, delta_t):
        ctx.B = B
        ctx.T = T
        ctx.C = C
        assert T <= T_MAX
        ...
        ctx.save_for_backward(w, u, k, v, delta_t)
        y = torch.empty((B, T, C), device='cuda', memory_format=torch.contiguous_format)
        wkv_cuda.forward(B, T, C, w, u, k, v, delta_t, y)
        if '32' in os.environ['RWKV_FLOAT_MODE']:
            return y
    ...
    def backward(ctx, gy):
        ...
        assert T <= T_MAX
        assert B * C % min(C, 1024) == 0
        w, u, k, v, delta_t = ctx.saved_tensors
        gw = torch.zeros((B, C), device='cuda').contiguous()
        gu = torch.zeros((B, C), device='cuda').contiguous()
        gk = torch.zeros((B, T, C), device='cuda').contiguous()
        gv = torch.zeros((B, T, C), device='cuda').contiguous()
        if '32' in os.environ['RWKV_FLOAT_MODE']:
            wkv_cuda.backward(B, T, C, w, u, k, v, delta_t, gy.contiguous(), gw, gu, gk, gv)
        else:
            wkv_cuda.backward(B, T, C, w, u, k, v, delta_t, gy.float().contiguous(), gw, gu, gk, gv)
...
def RUN_CUDA(B, T, C, w, u, k, v, delta_t):
    return WKV.apply(B, T, C, w.cuda(), u.cuda(), k.cuda(), v.cuda(), delta_t.cuda())
...
class RWKV_TimeMix(torch.jit.ScriptModule):
    ...
    def forward(self, x, delta_t):
        ...
        sr, k, v = self.jit_func(x, delta_t)

        rwkv = sr * RUN_CUDA(B, T, C, self.time_decay, self.time_first, k, v, delta_t)
...
class Block(nn.Module):
    ...
    def forward(self, x, delta_t):
        ...
        if self.layer_id == 0 and self.config.model_type == 'RWKV-ffnPre':
            ...
            x = x + self.att(self.ln1(x), delta_t)
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.gene_emb = nn.Linear(config.gene_dim, config.n_embd, bias=False)


    def forward(self, x_gene, delta_t, targets=None):

        self.step += 1
        B, T, C = x_gene.size()
        assert T <= self.ctx_len, "Cannot forward, because len(input) > model ctx_len."

        x = self.gene_emb(x_gene)
